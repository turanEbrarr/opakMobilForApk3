...
import 'package:opak_mobil_v2/irsaliye_raporlari/bekleyen_siparis_irsaliyeleri/bekleyen_siparis_irsaliyeleri_detay_rapor.dart';
import 'package:opak_mobil_v2/irsaliye_raporlari/irsaliyeler_pdf_onizleme.dart';
...
class bekleyen_siparis_irsaliyeleri_rapor_page extends StatefulWidget {
  final List<List<dynamic>> gelenBakiyeRapor;
  final List<bool> gelenFiltre;
  final Cari? cariKart;
  //final String titletemp;
  const bekleyen_siparis_irsaliyeleri_rapor_page(
      {super.key,
  ...
  @override
  State<bekleyen_siparis_irsaliyeleri_rapor_page> createState() =>
      _bekleyen_siparis_irsaliyeleri_rapor_pageState();
}

class _bekleyen_siparis_irsaliyeleri_rapor_pageState
    extends State<bekleyen_siparis_irsaliyeleri_rapor_page> {
        ...
        onLongPress: () async {
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return LoadingSpinner(
                color: Colors.black,
                message: "Bekleyen Sipariş İrsaliye Detayları Hazırlanıyor...",
              );
            },
          );

          List<bool> cek = await SharedPrefsHelper.filtreCek(
              "bekleyenSiparisIrsaliyeDetayRaporFiltre");
          ...
          if (gelen[0].length == 1 && gelen[1].length == 0) {
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: ((context) =>
                    bekleyen_siparis_irsaliyeleri_detay_rapor_page(
                      gelenFiltre: cek,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
                builder: (context) => irsaliyelerPdfOnizleme(
                      ...
                      faturaID: "-",
                      baslik: 'Bekleyen Sipariş İrsaliyeleri',
                    )),
          );
        },
      ),
      appBar: MyAppBar(
        height: 50,
        title: 'Bekleyen Sipariş İrsaliye Raporu',
      ),
      body: Column(
        children: [
          Column(
